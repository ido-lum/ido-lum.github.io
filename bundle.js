...
function convert(field) {
	...
	for (var i = 0, len = unique.length; i < len; i++) {
		//var col = (i * 10);
  		...
  		arrayUnique.push([unique[i], '1', fullColourHex(Math.floor(Math.random() * 256), Math.floor(Math.random() * 256), Math.floor(Math.random() * 256))]);

	}
	var ou = "";
	document.getElementById("output").innerHTML = ou; 
	//output to screen with colours
	for (var i = 0, len = arrayUnique.length; i < len; i++) {

  		var a = '<div style="background-color:#' + arrayUnique[i][2] + '">' + arrayUnique[i][0] + '</font> ';
...
},{"double-metaphone":1,"levenshtein-edit-distance":2}]},{},[3]);
